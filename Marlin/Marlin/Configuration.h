...
// Z2 is physically wired into the board's E1 driver socket (2nd Z motor,
// formerly parallel-wired to the Z driver, now on its own driver/socket).
 ...
 *
 * TODO: UNVERIFIED — inherited as-is from the stock Anet A8 example config.
 * This frame is heavily modified (per CLAUDE.md) and uses TMC2100 drivers whose
 * MS-jumper microstepping setting hasn't been physically confirmed. Do not trust
 * these values for a real print — pull the real values from the currently-operating
 * board's `M503` output (see backup/README.md) and/or measure belts/pulleys/leadscrew
 * directly before flashing this to replace the production board.
 */
...
// TODO: UNVERIFIED — stock Anet A8 dimensions, inherited from the example config.
// This frame is heavily modified; confirm actual bed size and travel limits
// (below) before trusting these for a real print.
